...
public class FireLevelInfoUI : IUserInterface
{
    private GameObject m_LevelInfoUI;
    private GameObject m_FireLevelInfo;
    ...
    private bool enterFireLevel1;
    private bool enterlevel;
    public FireLevelInfoUI(TowerOfAdventureGame TOAGame) : base(TOAGame)
    {
        Initialize();
    }
    ...
    public override void Initialize()
    {
        ...
        m_FireLevelInfo = m_LevelInfoUI.transform.GetChild(1).gameObject;
        BtnInit();
        FireLevelInfoClose();
        BattleReadyTeamInit();
    }
    ...
    private void BtnInit()
    {
        ...
        mBtn_return = m_FireLevelInfo.transform.GetChild(0).transform.GetChild(0).transform.GetChild(0).GetComponent<Button>();
        mBtn_return.onClick.AddListener(delegate ()
        {
            FireLevelInfoClose();
        });
        //mBtn_Level1 = UITool.FindGameComponent<Button>("WaterLevel1");
        mBtn_Level1 = m_FireLevelInfo.transform.GetChild(0).transform.GetChild(1).transform.GetChild(0).transform.GetChild(0).transform.GetChild(0).GetComponent<Button>();
        mBtn_Level1.onClick.AddListener(delegate () {
            BattleReadyTeamUIOpen();
            battleReadyTeamUIOpen = true;
        });
        m_BattleReadyTeamUI = m_FireLevelInfo.transform.GetChild(1).gameObject;
    ...
    public void FireLevelInfoOpen()
    {
        ...
        m_FireLevelInfo.SetActive(true);
    }

    /// <summary>
    /// 火關卡資訊關閉
    /// </summary>
    public void FireLevelInfoClose()
    {
        m_BattleReadyTeamUI.SetActive(false);
        m_FireLevelInfo.SetActive(false);
    ...
    private void TeamDetect()
    {
        ...
        if (Input.GetMouseButtonUp(0))
        {
            if (!redTeamItemOpen && allowEditorTeamOpen)
            {
                ...
                {
                    Debug.Log("點到的東西 : " + hit.transform.name);
